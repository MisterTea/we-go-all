...
static int is_rfc1918addr(const char* addr) {
  ...
  if (COMPARE(addr, "192.168.")) return 1;
  /* 10.0.0.0        -   10.255.255.255  (10/8 prefix) */
  if (COMPARE(addr, "10.")) return 1;
  /* 172.16.0.0      -   172.31.255.255  (172.16/12 prefix) */
  if (COMPARE(addr, "172.")) {
    int i = atoi(addr + 4);
    if ((16 <= i) && (i <= 31)) return 1;
  }
  return 0;
}
 ...
 *   pointer - OK
 *   NULL - error */
static char* simpleUPnPcommand2(SOCKET s, const char* url, const char* service,
  ...
  if (args == NULL) {
    soapbodylen =
        snprintf(soapbody, sizeof(soapbody),
                 ...
                 "<" SOAPPREFIX
                 ":Body>"
                 "<" SERVICEPREFIX ":%s xmlns:" SERVICEPREFIX
                 "=\"%s\">"
                 "</" SERVICEPREFIX
                 ":%s>"
                 "</" SOAPPREFIX ":Body></" SOAPPREFIX
                 ":Envelope>"
                 "\r\n",
                 action, service, action);
    if ((unsigned int)soapbodylen >= sizeof(soapbody)) return NULL;
  } else {
    char* p;
    const char *pe, *pv;
    ...
    *(p++) = ':';
    ...
    if (soapbody[sizeof(soapbody) -
                 1]) /* strncpy pads buffer with 0s, so if it doesn't end in 0,
                        could not fit full string */
      return NULL;
  }
...
#if 1
  buf =
      simpleUPnPcommand2((SOCKET)s, url, service, action, args, bufsize, "1.1");
#else
  buf =
      simpleUPnPcommand2((SOCKET)s, url, service, action, args, bufsize, "1.0");
  if (!buf || *bufsize == 0) {
#if DEBUG
    printf("Error or no result from SOAP request; retrying with HTTP/1.1\n");
#endif
    buf = simpleUPnPcommand2((SOCKET)s, url, service, action, args, bufsize,
                             "1.1");
  }
#endif
  return buf;
...
MINIUPNP_LIBSPEC struct UPNPDev* upnpDiscoverDevices(
    const char* const deviceTypes[], int delay, const char* multicastif,
    const char* minissdpdsock, int localport, int ipv6, unsigned char ttl,
    int* error, int searchalltypes) {
  struct UPNPDev* tmp;
  ...
  if (error) *error = UPNPDISCOVER_UNKNOWN_ERROR;
#if !defined(_WIN32) && !defined(__amigaos__) && !defined(__amigaos4__)
  /* first try to get infos from minissdpd ! */
  if (!minissdpdsock) minissdpdsock = "/var/run/minissdpd.sock";
...
#endif /* DEBUG */
        if (!strstr(minissdpd_devlist->st, "rootdevice")) only_rootdevice = 0;
        for (tmp = minissdpd_devlist; tmp->pNext != NULL; tmp = tmp->pNext) {
#ifdef DEBUG
          printf("returned by MiniSSDPD: %s\t%s\n", tmp->pNext->st,
                 tmp->pNext->descURL);
#endif /* DEBUG */
          if (!strstr(tmp->st, "rootdevice")) only_rootdevice = 0;
        }
        tmp->pNext = devlist;
        devlist = minissdpd_devlist;
        if (!searchalltypes && !only_rootdevice) break;
      }
    }
  }
  for (tmp = devlist; tmp != NULL; tmp = tmp->pNext) {
    ...
    if (!strstr(tmp->st, "rootdevice")) {
      if (error) *error = UPNPDISCOVER_SUCCESS;
      return devlist;
    }
  }
#endif /* !defined(_WIN32) && !defined(__amigaos__) && !defined(__amigaos4__) \
...
MINIUPNP_LIBSPEC struct UPNPDev* upnpDiscoverAll(
    int delay, const char* multicastif, const char* minissdpdsock,
...
MINIUPNP_LIBSPEC struct UPNPDev* upnpDiscoverDevice(
    const char* device, int delay, const char* multicastif,
    const char* minissdpdsock, int localport, int ipv6, unsigned char ttl,
    int* error) {
...
MINIUPNP_LIBSPEC int UPNP_GetValidIGD(struct UPNPDev* devlist,
  ...
  if (!desc) return -1; /* memory allocation error */
  /* Step 1 : downloading descriptions and testing type */
  for (dev = devlist, i = 0; dev; dev = dev->pNext, i++) {
    /* we should choose an internet gateway device.
     * with st == urn:schemas-upnp-org:device:InternetGatewayDevice:1 */
    desc[i].xml =
        miniwget_getaddr(dev->descURL, &(desc[i].size), myLanAddr,
                         sizeof(myLanAddr), dev->scope_id, &status_code);
#ifdef DEBUG
    if (!desc[i].xml) {
    ...
    if (desc[i].xml) {
      ...
      if (COMPARE(data->CIF.servicetype,
        ...
        desc[i].is_igd = 1;
        if (lanaddr) strncpy(lanaddr, myLanAddr, lanaddrlen);
      }
    }
  }
    ...
    for (dev = devlist, i = 0; dev; dev = dev->pNext, i++) {
      if (desc[i].xml) {
        ...
        if (desc[i].is_igd || state >= 3) {
          ...
          if (state >= 2) goto free_and_return;
  ...
  state = 0;
free_and_return:
  for (i = 0; i < ndev; i++) free(desc[i].xml);
  free(desc);
  return state;
}
...
int UPNP_GetIGDFromUrl(const char* rootdescurl, struct UPNPUrls* urls,
  ...
  descXML =
      miniwget_getaddr(rootdescurl, &descXMLsize, lanaddr, lanaddrlen, 0, NULL);
  if (descXML) {
    memset(data, 0, sizeof(struct IGDdatas));
    memset(urls, 0, sizeof(struct UPNPUrls));
